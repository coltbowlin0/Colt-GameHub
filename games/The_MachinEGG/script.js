(() => {
  const GAME_MARKET = "itchio"; // "itchio" o "crazygames" o "galaxy" o "newgrounds"
  ...
  function drawFence(ctx, w, h) {
    ...
    ctx.fillRect(0, topY + 7, w, 6);
    ctx.fillStyle = '#4a2e15';
    ctx.fillRect(0, topY + 27, w, 6);
    ctx.fillStyle = '#6b4423';
    ctx.fillRect(0, topY + 5, w, 6);
    ctx.fillStyle = '#6b4423';
    ctx.fillRect(0, topY + 25, w, 6);
    ctx.fillStyle = '#8b5a2b';
    ctx.fillRect(0, topY + 5, w, 2);
    ctx.fillStyle = '#8b5a2b';
    ctx.fillRect(0, topY + 25, w, 2);

    // Top fence vertical posts
    for (let x = 15; x < w; x += 55) {
      // Drop shadow
      ctx.fillStyle = 'rgba(0,0,0,0.3)';
      ctx.fillRect(x + 2, topY - 10, 14, 50);
      // Base post
      ctx.fillStyle = '#7a4e25';
      ctx.fillRect(x, topY - 10, 14, 48);
      ctx.beginPath();
      ctx.moveTo(x, topY - 10);
      ctx.lineTo(x + 7, topY - 18);
      ctx.lineTo(x + 14, topY - 10);
      ctx.fill();
      // Highlight post
      ctx.fillStyle = '#9b6a3b';
      ctx.fillRect(x, topY - 10, 3, 48);
      ctx.beginPath();
      ctx.moveTo(x, topY - 10);
      ctx.lineTo(x + 7, topY - 18);
      ctx.lineTo(x + 7, topY - 10);
      ctx.fill();
      // Detail / Nail
      ctx.fillStyle = '#2a1a0d';
      ctx.fillRect(x + 6, topY + 7, 2, 2);
      ctx.fillStyle = '#2a1a0d';
      ctx.fillRect(x + 6, topY + 27, 2, 2);
    }
  }
  ...
  function resolveTroughCollision(c) {
    ...
    let wx = -100,
      ww = 140 + padding;
    let wy = wyTrough - padding,
      wh = whTrough + padding * 2;
    ...
    if (c.x > wx && c.x < wx + ww && c.y > wy && c.y < wy + wh) {
      ...
      if (min === dl) {
        c.x = wx;
        if (!isTracking) c.velX = -Math.abs(c.velX || 10);
      } else if (min === dr) {
        c.x = wx + ww;
        if (!isTracking) c.velX = Math.abs(c.velX || 10);
      } else if (min === dt) {
        c.y = wy;
        if (!isTracking) c.velY = -Math.abs(c.velY || 10);
      } else {
        c.y = wy + wh;
        if (!isTracking) c.velY = Math.abs(c.velY || 10);
      }
    ...
    let fx = 760 - padding,
      fw = 200;
    let fy = fyTrough - padding,
      fh = fhTrough + padding * 2;
  ...
  function draw() {
    ...
    if (!window.pass1Queue) {
      window.pass1Queue = [];
      window.pass2Queue = [];
    }
    let pass1 = window.pass1Queue;
    pass1.length = 0;
    let pass2 = window.pass2Queue;
    pass2.length = 0;

    // Z-Index Pass 1: Background entities
    let mWLvl1 = state.maxWaterLevel || 0;
    let waterBaseY = 180 + (40 + mWLvl1 * 16) / 2 + 18;
    let mFLvl1 = state.maxFoodLevel || 0;
    let foodBaseY = 180 + (40 + mFLvl1 * 16) / 2 + 18;

    for (let i = 0; i < tombstonesArr.length; i++) {
      let t = tombstonesArr[i];
      t.rType = 4;
      let b = t.x < 400 ? waterBaseY : foodBaseY;
      if (t.y < b) pass1.push(t);
      else pass2.push(t);
    }
    for (let i = 0; i < chickensArr.length; i++) {
      let c = chickensArr[i];
      c.rType = 1;
      let b = c.x < 400 ? waterBaseY : foodBaseY;
      if (c.y < b) pass1.push(c);
      else pass2.push(c);
    }
    for (let i = 0; i < chicksArr.length; i++) {
      let ch = chicksArr[i];
      ch.rType = 2;
      let b = ch.x < 400 ? waterBaseY : foodBaseY;
      if (ch.y < b) pass1.push(ch);
      else pass2.push(ch);
    }
    for (let i = 0; i < roostersArr.length; i++) {
      let r = roostersArr[i];
      r.rType = 3;
      let b = r.x < 400 ? waterBaseY : foodBaseY;
      if (r.y < b) pass1.push(r);
      else pass2.push(r);
    }
    for (let i = 0; i < eggsArr.length; i++) {
      ...
      if (!e.collected && !e.isBeingDragged) {
        e.rType = 0;
        let b = e.x < 400 ? waterBaseY : foodBaseY;
        if (e.y < b) pass1.push(e);
        else pass2.push(e);
      }
    }
    ...
    let drawWaterPipe = (cy, rpp, isMax) => {
      ...
      ctx.fillStyle = '#c0392b';
      ctx.fillRect(0, cyOff - 8, pipeCenterX + 2, 16);
      ctx.fillStyle = '#e74c3c';
      ctx.fillRect(0, cyOff - 8, pipeCenterX + 2, 4); // Highlight
      ctx.fillStyle = '#922b21';
      ctx.fillRect(0, cyOff + 4, pipeCenterX + 2, 4); // Shadow

      // Vertical nozzle pointing down
      ctx.fillStyle = '#c0392b';
      ctx.fillRect(pipeCenterX - 10, cyOff - 8, 20, 24);
      ctx.fillStyle = '#e74c3c';
      ctx.fillRect(pipeCenterX - 10, cyOff - 8, 4, 24);
      ctx.fillStyle = '#922b21';
      ctx.fillRect(pipeCenterX + 6, cyOff - 8, 4, 24);

      // Silver cap
      ctx.fillStyle = '#bdc3c7';
      ctx.fillRect(pipeCenterX - 12, cyOff + 12, 24, 6);
      ctx.fillStyle = '#95a5a6';
      ctx.fillRect(pipeCenterX - 12, cyOff + 16, 24, 2);
      ...
      ctx.beginPath();
      ctx.ellipse(pipeCenterX, currentWaterFloor, 4 + splashPhase * 12, 1 + splashPhase * 4, 0, 0, Math.PI * 2);
      ctx.stroke();
    ...
    let depth = 18;
    ctx.fillStyle = '#777';
    ctx.fillRect(wx, wy + wh, ww, depth);
    ctx.strokeStyle = '#555';
    ctx.lineWidth = 2;
    ctx.strokeRect(wx, wy + wh, ww, depth);
    ctx.fillStyle = '#555';
    ...
    if (state.musicLevel > 0) {
      ...
      ctx.fillStyle = '#111';
      ctx.beginPath();
      ctx.arc(rx + 10, ry + 10, 6, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(rx + rw - 10, ry + 10, 6, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#fff';
    ...
    let drawFoodPipe = (cy, rpp, isMax) => {
      ...
      ctx.fillRect(pipeCenterX - 2, cyOff - 8, drawW, 16);
      ctx.fillStyle = '#f39c12';
      ctx.fillRect(pipeCenterX - 2, cyOff - 8, drawW, 4); // Highlight
      ctx.fillStyle = '#d35400';
      ctx.fillRect(pipeCenterX - 2, cyOff + 4, drawW, 4); // Shadow
    ...
    ctx.fillRect(fx, fy + fh, fw, fDepth);
    ctx.strokeStyle = '#2d1c11';
    ctx.lineWidth = 2;
    ctx.strokeRect(fx, fy + fh, fw, fDepth);
    ...
    if (state.hasStamper) {
      ...
      ctx.beginPath();
      ctx.arc(400, basY - stamperH + 12, 5, 0, Math.PI * 2);
      ctx.fill();
  ...
  canvas.addEventListener('pointerdown', (e) => {
    if (cinematicPhase >= 4 && window._speedrunCanvasBtn) {
      ...
      if (pos.x >= b.x && pos.x <= b.x + b.w && pos.y >= b.y && pos.y <= b.y + b.h) {
        ...
        try {
          window.CrazyGames.SDK.data.removeItem('chickenIdleSave');
        } catch (err) {}
        location.reload();
        return;
      }
    ...
    for (let i = tombstonesArr.length - 1; i >= 0; i--) {
      ...
      if (Math.abs(dx) < 15 && Math.abs(dy) < 15) {
        ...
        if (t.hp <= 0) {
          ...
          for (let j = 0; j < 5; j++) particlesArr.push({
  ...
  document.getElementById('wipe-confirm').addEventListener('click', async () => {
    ...
    if (typeof GAME_MARKET !== 'undefined' && GAME_MARKET === "crazygames" && window.isCrazyGamesInitialized) {
      try {
        await window.CrazyGames.SDK.data.removeItem('chickenIdleSave');
      } catch (e) {}
      try {
        await window.CrazyGames.SDK.data.setItem('chickenIdleSave', '');
      } catch (e) {}
      try {
        if (window.CrazyGames.SDK.data.clear) await window.CrazyGames.SDK.data.clear();
      } catch (e) {}
  ...
  function updateCinematic(dt) {
    ...
    // Phase 2: Formation
      ...
      if (allInPosition || cinematicTimer > 15) {
        cinematicPhase = 3;
        cinematicTimer = 0;
      }
    }
  ...
  function saveState() {
    ...
    if (GAME_MARKET === "crazygames" && window.isCrazyGamesInitialized) {
      try {
        window.CrazyGames.SDK.data.setItem('chickenIdleSave', saveString);
      } catch (e) {}
    }
  }
      ...
      fetch(RANKING_API_URL + '/submit_score.php', {
        ...
        .then(data => {
          if (data.ok) {
            ...
            localStorage.setItem('chickenIdleSpeedrunData', JSON.stringify({
              pos: data.pos,
              name: name,
              market: selectedMarket,
              time_secs: timeSecs
